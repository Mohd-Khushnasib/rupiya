                        ...
                        success: function (data) {
                            ...
                            if (data.data.length > 0) {
                                ...
                                result.forEach((item, index) => {
                                    ...
                                    // Fix 1: Handle case sensitivity and whitespace issues in status comparison
                                    const normalizeStatus = (status) => {
                                        return status ? status.trim() : 'NEW EMPLOYEE';
                                    };

                                    const statusDropdown = `
                                <div class="form-group">
                                    <select class="form-control leads_system_bg employeeStatusSelect"
                                        data-id="${item.id}" data-selected="${normalizeStatus(item.employee_status)}"
                                        data-row="${index + 1}" tabindex="1">
                                        ${statusOptions.map(status => {
                                                        // Normalize both the database value and the option value
                                                        const dbStatus = normalizeStatus(item.employee_status);

                                                        // Exact match comparison - case sensitive but trimmed whitespace
                                                        const isSelected = dbStatus === status;
